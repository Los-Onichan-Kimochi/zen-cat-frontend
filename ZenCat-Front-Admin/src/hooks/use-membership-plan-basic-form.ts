...
export const membershipPlanFormSchema = z
  .object({
    ...
    fee: z
      .number()
      .min(10, { message: 'El precio debe ser un número mayor o igual a 10' }),
    has_limit: z.string({
  ...
  .superRefine((data, ctx) => {
    if (
      data.has_limit === 'true' &&
      (data.reservation_limit === null || data.reservation_limit === undefined)
    ) {
      ctx.addIssue({
    ...
    if (data.has_limit === 'false' && data.reservation_limit != null) {
      ctx.addIssue({
        ...
        code: z.ZodIssueCode.custom,
        message:
          'No debe establecer un límite si no se ha activado la opción de límite.',
      });
    }
  });
