  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const user = row.original;
        const phone =
          user.phone || user.onboarding?.phoneNumber || 'No disponible';
        return (
    ...
    },
    {
      id: 'location',
      accessorFn: (row) =>
        `${row.city || row.onboarding?.city || ''} ${row.district || row.onboarding?.district || ''}`.trim(),
      header: ({ column }) => (
      ...
      cell: ({ row }) => {
        ...
        const location =
          [city, district].filter(Boolean).join(', ') || 'No disponible';
